...
## Load the data
### HbA1c
ghb_data_pre <- read.csv(paste0(path_to_data,"L10_C.csv"))
str(ghb_data_pre) # SEQN/LBXGH
ghb_data <- ghb_data_pre %>% select(c("SEQN","LBXGH")) %>% as.data.table()
str(ghb_data)

### Self-reported DM
dmq_data_pre <- read.csv(paste0(path_to_data,"DIQ_C.csv"))
str(dmq_data_pre)
dmq_data <- dmq_data_pre %>% select(c("SEQN", "DIQ010", "DIQ050", "DIQ070")) %>% as.data.table()
str(dmq_data)

### Fasting glucose
fgl_data_pre <- read.csv(paste0(path_to_data,"L10AM_C.csv"))
str(fgl_data_pre)
...
demo_data_pre <- read.csv(paste0(path_to_data,"DEMO_C.csv"))
str(demo_data_pre) 
...
### Medical conditions
med_data_pre <- read.csv(paste0(path_to_data,"MCQ_C.csv"))
str(med_data_pre) 
...
### LDL-C
ldl_data_pre <- read.csv(paste0(path_to_data,"L13AM_C.csv"))
str(ldl_data_pre) 
...
### HDL-C
hdl_data_pre <- read.csv(paste0(path_to_data,"L13_C.csv"))
str(hdl_data_pre) 
hdl_data <- hdl_data_pre %>%
  select(c("SEQN", "LBXHDD")) %>%
  as.data.table()
str(hdl_data)

### Blood pressure
bp_data_pre <- read.csv(paste0(path_to_data,"BPX_C.csv"))
str(bp_data_pre) 
bp_data <- bp_data_pre %>%
  select(c("SEQN", 
...
smk_data_pre <- read.csv(paste0(path_to_data,"SMQ_C.csv"))
str(smk_data_pre) 
...
### BMI
bmi_data_pre <- read.csv(paste0(path_to_data,"BMX_C.csv"))
str(bmi_data_pre) 
...
kdn_lvr_data_pre <- read.csv(paste0(path_to_data,"L40_C.csv"))
...
cardio_data_pre <- read.csv(paste0(path_to_data,"CDQ_C.csv"))
str(cardio_data_pre) 
...
### Prescription drug
prescrpt_data_pre <- read.csv(paste0(path_to_data,"RXQ_RX_C.csv"))
...
table(prescrpt_data$RXDDRUG) # N's of Unknow/Refused/Don't know: 13/11/53
(13+11+53)/nrow(prescrpt_data) # 0.004184555
...
# save as a csv file
write.csv(data_pre, paste0(path_to_output,"nhanes0304_sens_with_variables_20240111.csv"))