# Input
# NHANES data (2005-2006) in csv format:
# Output
# Variables to be used
# nhanes0506_sens_with_variables_20240111.csv
# R4.1.3
...
## Load the data
### HbA1c
ghb_data_pre <- read.csv(paste0(path_to_data,"GHB_D.csv"))
str(ghb_data_pre) # SEQN/LBXGH
ghb_data <- ghb_data_pre %>% select(c("SEQN","LBXGH")) %>% as.data.table()
str(ghb_data)

### Self-reported DM
dmq_data_pre <- read.csv(paste0(path_to_data,"DIQ_D.csv"))
str(dmq_data_pre)
dmq_data <- dmq_data_pre %>% select(c("SEQN", "DIQ010", "DIQ050", "DID070")) %>% as.data.table()
str(dmq_data)

### Fasting glucose
fgl_data_pre <- read.csv(paste0(path_to_data,"GLU_D.csv"))
str(fgl_data_pre)
...
demo_data_pre <- read.csv(paste0(path_to_data,"DEMO_D.csv"))
str(demo_data_pre) 
...
### Medical conditions
med_data_pre <- read.csv(paste0(path_to_data,"MCQ_D.csv"))
str(med_data_pre) 
...
### LDL-C
ldl_data_pre <- read.csv(paste0(path_to_data,"TRIGLY_D.csv"))
str(ldl_data_pre) 
...
### HDL-C
hdl_data_pre <- read.csv(paste0(path_to_data,"HDL_D.csv"))
str(hdl_data_pre) 
...
### Blood pressure
bp_data_pre <- read.csv(paste0(path_to_data,"BPX_D.csv"))
str(bp_data_pre) 
bp_data <- bp_data_pre %>%
  select(c("SEQN", 
...
smk_data_pre <- read.csv(paste0(path_to_data,"SMQ_D.csv"))
str(smk_data_pre) 
...
### BMI
bmi_data_pre <- read.csv(paste0(path_to_data,"BMX_D.csv"))
str(bmi_data_pre) 
...
kdn_lvr_data_pre <- read.csv(paste0(path_to_data,"BIOPRO_D.csv"))
...
cardio_data_pre <- read.csv(paste0(path_to_data,"CDQ_D.csv"))
str(cardio_data_pre) 
...
### Prescription drug
prescrpt_data_pre <- read.csv(paste0(path_to_data,"RXQ_RX_D.csv"))
...
table(prescrpt_data$RXDDRUG) # N's of Unknow/Refused/Don't know: 2/13/47
(2+13+47)/nrow(prescrpt_data) # 0.003443488
...
# save as a csv file
write.csv(data_pre, paste0(path_to_output,"nhanes0506_sens_with_variables_20240111.csv"))