...
# NHANES 2001-2002 with necessary variables: 
...
nhanes_pre <- read.csv(paste0(path_to_data,"nhanes0102_sens_with_variables_20240111.csv"))
...
table(nhanes_pre$t2dm, exclude = NULL) # T2DM: Yes/No = 681/10358
...
table(is.na(data_pre$LBXGH)) # TRUE/FALSE = 65/616
table(is.na(data_pre$LBXGLU)) # TRUE/FALSE = 347/334
...
data[(MCQ160C == 1| MCQ160D == 1| MCQ160E == 1| MCQ160F == 1), ] %>% nrow() #68
...
(53+9)/nrow(data) # 0.1856287 (prop. of those with current smoking)
...
summary(data$LBDHDL)
data[(RIAGENDR == 1 & LBDHDL < 40)|(RIAGENDR == 2 & LBDHDL < 50)|
       is.na(LBDHDL), # Not exclude those with missing HDL value
     A5_weak_compC_hdl:=1]
...
### HDL
summary(data$LBDHDL)
data[(LBDHDL < 55 & (RIAGENDR == 2 | RIDRETH1 == 4))|
       (LBDHDL <= 50 & RIAGENDR != 2 & RIDRETH1 != 4)|
       is.na(LBDHDL), # Not exclude those with missing HDL value
     A6_weak_lipid_hdl:=1]
...
# save as a csv file
write.csv(data, paste0(path_to_output,"data_eligibility_sens_2001_20240111.csv"))
