# Input
# NHANES data (2003-2004) in csv format:
# NHANES 2017-2018 with necessary variables: 
# nhanes0304_sens_with_variables_20240111.csv
# R4.1.3
...
nhanes_pre <- read.csv(paste0(path_to_data,"nhanes0304_sens_with_variables_20240111.csv"))
...
table(nhanes_pre$t2dm, exclude = NULL) # T2DM: Yes/No = 687/9435
...
table(is.na(data_pre$LBXGH)) # TRUE/FALSE = 61/626
table(is.na(data_pre$LBXGLU)) # TRUE/FALSE = 374/313
...
# data[(MCQ160C == 1| MCQ160D == 1| MCQ160E == 1), A4_weak_cvd:=1]
# data[(MCQ160C == 1| MCQ160D == 1| MCQ160E == 1), ] %>% nrow()
data[(MCQ160C == 1| MCQ160D == 1| MCQ160E == 1| MCQ160F == 1), A4_weak_cvd:=1]
data[(MCQ160C == 1| MCQ160D == 1| MCQ160E == 1| MCQ160F == 1), ] %>% nrow() #82
...
summary(data$LBXHDD) # HDL-C
summary(data$BPXDI1) # DBP
summary(data$BPXSY1) # SBP
...
(47+13)/nrow(data) # 0.1916933 (prop. of those with current smoking)

summary(data$BMXBMI) # BMI

### mean SBP and DBP
data <- data %>% 
  mutate(meanSBP = rowMeans(.[,c("BPXSY1", "BPXSY2", "BPXSY3")], na.rm=TRUE),
         meanDBP = rowMeans(.[,c("BPXDI1", "BPXDI2", "BPXDI3")], na.rm=TRUE))
...
summary(data$LBXHDD)
data[(RIAGENDR == 1 & LBXHDD < 40)|(RIAGENDR == 2 & LBXHDD < 50)|
       is.na(LBXHDD), # Not exclude those with missing HDL value
     A5_weak_compC_hdl:=1]
...
data[(BMXBMI >= 32|is.na(BMXBMI)), A5_weak_compC_bmi:=1] #124  # Not exclude those with missing BMI values
...
### HDL
summary(data$LBXHDD)
data[(LBXHDD < 55 & (RIAGENDR == 2 | RIDRETH1 == 4))|
       (LBXHDD <= 50 & RIAGENDR != 2 & RIDRETH1 != 4)|
       is.na(LBXHDD), # Not exclude those with missing HDL value
     A6_weak_lipid_hdl:=1]
...
# save as a csv file
write.csv(data, paste0(path_to_output,"data_eligibility_sens_2003_20240111.csv"))
