...
nhanes_pre <- read.csv(paste0(path_to_data,"nhanes0506_sens_with_variables_20240111.csv"))
...
table(nhanes_pre$DID070, exclude = NULL) #1/2/9 = Yes/No/Don't know

nhanes_pre[((DIQ010==1|LBXGH >= 6.5|LBXGLU >= 126|
               DIQ050==1|DID070==1) & RIDAGEYR >= 20), t2dm:=1]
...
table(data_pre$DID070, exclude = NULL) #1/2/9 = Yes/No/Don't know
...
table(is.na(data_pre$LBXGH)) # TRUE/FALSE = 58/607
table(is.na(data_pre$LBXGLU)) # TRUE/FALSE = 351/314
...
data[(MCQ160C == 1| MCQ160D == 1| MCQ160E == 1| MCQ160F == 1), ] %>% nrow() #75
...
(52+4)/nrow(data) # 0.1794872 (prop. of those with current smoking)
...
# save as a csv file
write.csv(data, paste0(path_to_output,"data_eligibility_sens_2005_20240111.csv"))
